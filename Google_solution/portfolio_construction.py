...
from Data_generation import prices_df, tickers, ETF_df
...
# copy_features = fundamental.copy()
...
#np.seterr(divide='ignore',invalid='ignore')
       ...
       # Creating z-score
        ...
        if self.ptype == 1:   
            ...
            for i, value in enumerate(self.df.iterrows()):
                ...
                self.df.iloc[i]['Long Market'] = value[1]['Longs']#seweklylf.long_market
                self.df.iloc[i]['Short Market'] = value[1]['Shorts']#self.short_market
    ...
    def create_returns(self, allocation,pair_number):
        if self.ptype==1:
            ...
            self.portfolio['X'] = self.df['X'] * self.portfolio['Positions']
...
bottom_up, bottom_up_sharpe, bottom_up_sigma = bot_up_opti(cointegrated, regime_predictions, X_test, 90000)
...
def etf_strat(ETF_df, investment, rate = 0.015):
    etf_portfolio = pd.DataFrame()
    weights_etf = [0.06, 0.1, 0.3, 0.08, 0.16, 0.3]
    
    for count, ticker in enumerate(ETF_df):
        value = investment * weights_etf[count] * (ETF_df[ticker] / ETF_df[ticker].shift(1)).cumprod()
        etf_portfolio[ticker] = value
    
    etf_portfolio['Cash'] = 10000
    etf_portfolio['Total Portfolio Value'] =   etf_portfolio.sum(axis = 1)
    etf_portfolio['Returns'] = np.log(etf_portfolio['Total Portfolio Value'] / etf_portfolio['Total Portfolio Value'].shift(1))
    etf_portfolio = etf_portfolio.fillna(0)
    
    
    etf_portfolio_mu = etf_portfolio['Returns'].mean()
    etf_portfolio_sigma = etf_portfolio['Returns'].std()
    #recall that we initialized our interest assumption earlier
    etf_portfolio_sharpe = (etf_portfolio_mu - 0.015) / etf_portfolio_sigma

# =============================================================================
#     plt.figure(figsize = (10,6))
#     plt.plot(etf_portfolio['Total Portfolio Value'])
#     plt.title('ETF Portfolio Equity Curve')
#     plt.show() 
# =============================================================================

    return etf_portfolio, etf_portfolio_sharpe 

etf_portfolio, etf_portfolio_sharpe = etf_strat(ETF_df, 90000)

def analyze(spo_portfolio, spo_portfolio_sharpe, bottom_up, bottom_up_sharpe, p_efficient_frontier, p_efficient_frontier_sharpe, equal, equal_sharpe, etf_portfolio, etf_portfolio_sharpe):
    #list to hold portfolio names
    names = ['Equally Weighted','Efficient Frontier','Bottom Up','SPO Framework', 'ETF Portfolio']
    #variable to hold column name
    column_name = 'Sharpe Ratio'
    #list to hold Sharpe Ratios
    sharpes = [equal_sharpe, p_efficient_frontier_sharpe, bottom_up_sharpe, spo_portfolio_sharpe, etf_portfolio_sharpe]
    ...
    portfolio_values = [equal['Total Portfolio Value'].tail(1).values.astype(int), p_efficient_frontier['Total Portfolio Value'].tail(1).values.astype(int), bottom_up['Total Portfolio Value'].tail(1).values.astype(int),spo_portfolio['Total Portfolio Value'].tail(1).values.astype(int),etf_portfolio['Total Portfolio Value'].tail(1).values.astype(int)]
